...
import { safeFormSchema, type SafeFormSchema } from "@/lib/schema";
import type { SafeWithSections } from "@/lib/services/safe-service";

interface SafeFormProps {
  initialData?: SafeWithSections | null;
}
...
export function SafeForm({ initialData }: SafeFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [previews, setPreviews] = useState<{ [key: string]: string }>({});

  const form = useForm<SafeFormSchema>({
  ...
  const handleSubmit = async (data: SafeFormSchema) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Safe information updated successfully!");
      } else {
        throw new Error(result.error || "Failed to update Safe information");
      }
    } catch (error) {
      console.error("Error submitting Safe form:", error);
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to update Safe information"
      );
    } finally {
