...
import { preciseFormSchema, type PreciseFormSchema } from "@/lib/schema";
import type { PreciseWithSections } from "@/lib/services/precise-service";

interface PreciseFormProps {
  initialData?: PreciseWithSections | null;
}
...
export function PreciseForm({ initialData }: PreciseFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [previews, setPreviews] = useState<{ [key: string]: string }>({});

  const form = useForm<PreciseFormSchema>({
    resolver: zodResolver(preciseFormSchema),
    defaultValues: {
  ...
  const handleSubmit = async (data: PreciseFormSchema) => {
    ...
    try {
      ...
      const response = await fetch("/api/precise", {
        method: "PUT",
        body: formData,
      });
      ...
      if (result.success) {
        toast.success("Precise information updated successfully!");
      } else {
        throw new Error(result.error || "Failed to update Precise information");
      }
    } catch (error) {
      console.error("Error submitting Precise form:", error);
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to update Precise information"
      );
    } finally {
