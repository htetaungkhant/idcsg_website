...
export default async function Safe() {
  ...
  if (!safe || !safe.sections || safe.sections.length <= 0) {
    return (
      <div className="min-h-[75vh] flex items-center justify-center">
        <p className="text-2xl text-gray-600">No safe information available.</p>
      </div>
    );
  }

  // Render the safe information using CardCollection components
  return (
      ...
      <div className="flex flex-col mt-20">
        {safe.sections.map((section, index) =>
          ...
          section.cardStyle === "CARDSTYLE3" ? (
            ...
            >
              <p className="font-(family-name:--font-roboto) whitespace-pre-wrap">
                {section.description}
              </p>
            </CardCollectionStyle1>
