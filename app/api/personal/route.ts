...
/**
 * GET /api/personal
 * Retrieve the single Personal record with its sections
 */
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const result = await PersonalService.createOrUpdatePersonal(personalData);
...
export async function DELETE() {
  try {
    ...
    const result = await PersonalService.deletePersonal();
    ...
    if (result) {
      ...
      return NextResponse.json(
        { success: false, error: "Failed to delete Personal" },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error("DELETE /api/personal error:", error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
